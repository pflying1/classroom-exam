function exampleOne(numberValue) {
  ...
  // --------호출 부분 -------
  integerCheck(numberValue, function (err, data) {
    if (err) {
      console.log(err);
    } else {
      innerOne(data, function (err, first) {
        if (err) {
          console.log(err);
        } else {
          innerTwo(first, function (err, second) {
            if (err) {
              console.log(err);
            } else {
              innerThree(second, function (err, third) {
                if (err) {
                  console.log(err);
                } else {
                  innerFour(third, function (err, fourth) {
                    if (err) {
                      console.log(err);
                    } else {
                      console.log(fourth);
